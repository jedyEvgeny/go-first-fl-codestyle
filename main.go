...
func defence(charName, charClass string) (str string) {
	if charClass == "warrior" {
		str = fmt.Sprintf("%s блокировал %d урона.", charName, 10+randint(5, 10))
	}
	if charClass == "mage" {
		str = fmt.Sprintf("%s блокировал %d урона.", charName, 10+randint(-2, 2))
	}
	if charClass == "healer" {
		str = fmt.Sprintf("%s блокировал %d урона.", charName, 10+randint(2, 5))
	}
	if charClass == "healer" {
		str = "неизвестный класс персонажа"
	}
	return str
}

// обратите внимание на "if else" и на "else"
func special(charName, charClass string) (str string) {
	if charClass == "warrior" {
		str = fmt.Sprintf("%s применил специальное умение `Выносливость %d`", charName, 80+25)
	}
	if charClass == "mage" {
		str = fmt.Sprintf("%s применил специальное умение `Атака %d`", charName, 5+40)
	}
	if charClass == "healer" {
		str = fmt.Sprintf("%s применил специальное умение `Защита %d`", charName, 10+30)
	}
	if charClass == "healer" {
		str = "неизвестный класс персонажа"
	}
...
func choiseCharClass() string {
	...
	for appriveChoice != "y" {
		...
		fmt.Scanf("%s\n", &appriveChoice)
		appriveChoice = strings.ToLower(appriveChoice)
	}
	return charClass
}
